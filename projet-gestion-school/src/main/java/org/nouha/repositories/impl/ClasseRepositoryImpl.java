...
public class ClasseRepositoryImpl implements ClasseRepository {
    ...
    private final String SQL_INSERT = "INSERT INTO Classe(libelleClasse,salle_id, cour_id, archive) VALUES (?, ?, ?, ?)";
    private final String SQL_UPDATE = "UPDATE Classe SET libelleClasse = ?, salle_id = ?, cour_id = ?, archive = ? WHERE id = ?";
    private final String SQL_SELECT_BY_CLASSE= "SELECT * FROM `Classe` WHERE id = ?";
    private final String SQL_ARCHIVE = "UPDATE Classe SET archive = ? WHERE id = ?";
     private final String SQL_SELECT_ALL_MODULES_BY_CLASSE_ID="SELECT * FROM Modules WHERE classe_id =?";
...
public void update(Classe data) {
    try {
        ...
        database.getPs().setInt(3, data.getSalle().getId());
        database.getPs().setInt(4, data.getCour().getId());
    ...
    try {
        ...
        while (resultSet.next()) {
            ...
            Classe classe = new Classe(classeId, libelleClasse, archive,salle, cour);
            classesList.add(classe);
        }
        resultSet.close();
        database.closeConnexion();
    ...
    public boolean modifierClasse(Classe classe) {
        try {
            ...
            database.getPs().setDouble(2, classe.getSalle().getId());
            database.getPs().setInt(3, classe.getCour().getId());
            database.getPs().setBoolean(4, classe.isArchive());
            database.getPs().setInt(5, classe.getId());
